...
#### Seçenek 1: Binary İndirme (Önerilen / Recommended)

Web/tarayıcı imzası için hafif **Rust Agent Lite** (tek dosya, ~2.9 MB, .NET gerektirmez)
public MBox'tan indirilir — indirme ve kurulum sayfası:
**https://github.com/moreum-tech/MBox/tree/main/digimr/agent**

```bash
# Windows (şu an yalnız Windows destekleniyor)
# digimr-agent-win-x64.exe indirin, çift tıklayıp çalıştırın — unzip/kurulum yok.

# macOS / Linux: native PIN penceresi henüz yok — yakında (Plan 3).
#   Bu platformlarda şimdilik "Seçenek 2: Kaynak Koddan" ile .NET Agent'ı derleyin.
```
    ...
    const response = await fetch('http://localhost:5555/api/eyp/create', {
        ...
        body: JSON.stringify({
            ...
            ustveri: {
                ...
                guvenlikKodu: 'HZO', // gecerli kod (V2.x: YOK/HZO/OZL/GZL/CGZ; V1.3: TSD/HZO/OZL/GZL/CGZ/KSO) — etiket DEGIL
