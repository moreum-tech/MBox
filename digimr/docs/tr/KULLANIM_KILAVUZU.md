...
Web/tarayici imzasi icin hafif **Rust Agent Lite** (2.9 MB, tek dosya, .NET gerektirmez)
public MBox'tan indirilir — indirme + kurulum sayfasi:

- https://github.com/moreum-tech/MBox/tree/main/digimr/agent
- Windows (su an yalniz Windows): `digimr-agent-win-x64.exe` indir, cift tikla calistir (unzip yok).
- macOS / Linux: native PIN penceresi henuz yok — yakinda (Plan 3).

Agent varsayilan olarak `http://localhost:5555` adresinde calisir. API'si .NET Agent ile aynidir.
...
### 3.2 PDF Imzalama — `SignDataWithProviderAsync`
...
### 3.4 XML Imzalama — `SignDataWithProviderAsync`
...
```csharp
var xmlData = File.ReadAllBytes("fatura.xml");
var result = await sdk.SignDataWithProviderAsync(xmlData, provider, new SignatureParameters
{
...
```csharp
var documentData = File.ReadAllBytes("imzali.pdf");
var result = await sdk.ValidateDocumentAsync(documentData);
...
### 3.7 EYP Paketi — `CreateEypPackageV21Async`

```csharp
Task<EypCreateResult> CreateEypPackageV21Async(
...
curl -X POST http://localhost:7701/api/v1/token/sessions \
  -H "Content-Type: application/json" \
  -d "{\"libraryPath\": \"C:/Windows/System32/eTPKCS11.dll\", \"pin\": \"$TOKEN_PIN\", \"slotId\": 0}"
...
> **⚠️ Ubuntu 24.04+ / OpenSSL 3.x Uyarisi:** SoftHSM2 PIN decrypt icin OpenSSL **legacy provider** (3DES/DES) gerektirir. Ubuntu 24.04+ varsayilan olarak legacy'yi kapali getirir; bu durumda login ilk seferde calisir ama sonraki session'larda `CKR_PIN_INCORRECT` verir (aslinda crypto init hatasi, PIN dogru).
>
> **Cozum:** `/etc/ssl/openssl.cnf` icinde legacy provider'i aktifle:
>
> ```ini
> [openssl_init]
...
> ```
>
> **Onemli:** Bu degisikligi ASLA sadece SSH uzerinden yapmayin — sshd restart sirasinda OpenSSL init fail edebilir ve sunucuya erisimi kaybedebilirsiniz. Konsol/KVM hazir olmali, degisiklikten once backup alin (`cp openssl.cnf openssl.cnf.bak`), `openssl version -a` ile valide edin, **yeni** SSH session acip test edin (mevcut session canli kalir).
>
> **Dogrulama:** `openssl list -providers` ciktisinda hem `default` hem `legacy` gorunmeli. Sonra `softhsm2-util --show-slots` hata vermeden slot listesi donmeli.
>
> **Etkilenmeyen:** Windows dev makineler (SoftHSM Windows installer kendi OpenSSL'ini getirir), Ubuntu 22.04 ve alti (default legacy aktif), eToken/AKiS gibi donanim token'lar (kendi PKCS#11 kutuphanesi, OpenSSL bagimliligi yok).
...
> **Uyari:** Uretim ortaminda PIN'i asla plain-text olarak yazmayin. Client uygulamasi tarafinda ortam degiskeninden (`TOKEN_PIN`) okuyun veya guvenli giris mekanizmasi kullanin.
...
*Detayli API referansi icin: [API_REFERANS.md](API_REFERANS.md)*
*SDK tip referansi icin: [SDK_REFERANS.md](SDK_REFERANS.md)*
