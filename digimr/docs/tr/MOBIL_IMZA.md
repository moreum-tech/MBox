...
|------|---------|----------|
| `type` | ✅ | `"mobile"` |
| `msisdn` | ✅ | Abonenin telefon numarası. `905XXXXXXXXX`, `+90...` veya ulusal `05XXXXXXXXX` kabul edilir; Turkcell için DigiMR otomatik olarak ulusal formata (`05XXXXXXXXX`) çevirir |
...
C: `905XXXXXXXXX`, `+905XXXXXXXXX` veya ulusal `05XXXXXXXXX` gönderebilirsiniz. **Turkcell MSSP ulusal formatı (`05XXXXXXXXX`, baştaki `0`) bekler**; DigiMR Turkcell için numarayı otomatik bu formata çevirir (aksi halde MSSP `UNKNOWN_CLIENT` döner). Operatör numaradan **çıkarılmaz** — numara taşıma (MNP) nedeniyle önek operatörü belirlemez; `mobileOperator` her zaman açıkça belirtilmelidir.
